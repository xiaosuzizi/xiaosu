...
let loadedAwemeIds = new Set(); // 记录已添加到左侧列表的抖音号
...
async function loadAllAwemeVideos() {
    ...
    if (container) {
        container.innerHTML = '<div style="grid-column:1/-1; text-align:center; padding:40px; color:#1890ff;">正在加载...</div>';
    }

    allAwemeVideos = [];
    loadedAwemeIds.clear();
    ...
    // ===== 并行加载所有配置的第1页（核心提速点） =====
    const firstPagePromises = queryConfigs.map(async (config) => {
        let url = `/api/local_material/video/aweme/get?local_account_id=${localAccountId}&anchor_types=${config.anchor_types}&cursor=0&page_size=100&order_filed=${orderField}`;
        ...
        try {
            const res = await fetch(url);
            const data = await res.json();
            if (data.code === 0 && data.data?.video_list) {
                return {
            ...
            console.error('加载第1页失败', e);
        }
        return { list: [], cursor: '0', hasMore: false, config };
    });

    const firstPageResults = await Promise.all(firstPagePromises);

    // 合并第1页数据
    firstPageResults.forEach(r => {
        allAwemeVideos = allAwemeVideos.concat(r.list);
    });
    ...
    // 立即显示第1页（不等后台加载完）
    fillAwemeSelectFromVideos(allAwemeVideos, false);
    modalVideoList = allAwemeVideos.slice(0, 100);
    modalVideoHasMore = firstPageResults.some(r => r.hasMore) || allAwemeVideos.length > 100;
    modalVideoCursor = '0';
    modalVideoCurrentPageIndex = 0;
    ...
    // ===== 后台异步加载剩余页面（用户无感知） =====
    loadRemainingPagesAsync(firstPageResults, orderField, localAccountId);
}

// 后台异步加载剩余页面
async function loadRemainingPagesAsync(firstPageResults, orderField, localAccountId) {
    const promises = firstPageResults
        .filter(r => r.hasMore)
        .map(async (r) => {
            let cursor = r.cursor;
            ...
            while (hasMore && page < 100) {
                let url = `/api/local_material/video/aweme/get?local_account_id=${localAccountId}&anchor_types=${config.anchor_types}&cursor=${cursor}&page_size=100&order_filed=${orderField}`;
                if (config.poi_ids && config.anchor_types === 'POI_ANCHOR') {
                    const arr = config.poi_ids.split(',').map(x => x.trim()).filter(x => x);
                    url += `&poi_ids=${encodeURIComponent(JSON.stringify(arr))}`;
                }
                ...
                try {
                    ...
                    if (data.code === 0 && data.data?.video_list) {
                        ...
                        // 过滤已存在的视频
                        const existingIds = new Set(allAwemeVideos.map(v => v.video_id));
                        const newList = list.filter(v => !existingIds.has(v.video_id));
                        
                        if (newList.length > 0) {
                            allAwemeVideos = allAwemeVideos.concat(newList);
                            // 追加新抖音号到左侧（动态更新，用户可见）
                            fillAwemeSelectFromVideos(newList, true);
                        }
...
function fillAwemeSelectFromVideos(videoList, append = false) {
    ...
    videoList.forEach(item => {
        if (item.aweme_id && !loadedAwemeIds.has(item.aweme_id)) {
            loadedAwemeIds.add(item.aweme_id);
            
            const label = document.createElement('label');
            label.style.cssText = 'display:flex; align-items:flex-start; gap:6px; padding:6px 8px; cursor:pointer; border-radius:4px; transition:background 0.2s; font-size:13px; color:#333;';
            label.onmouseover = function() { this.style.background = '#f5f5f5'; };
            label.onmouseout = function() { this.style.background = 'transparent'; };
            
            label.innerHTML = `
                <input type="checkbox" class="aweme-filter-checkbox" value="${item.aweme_id}" style="cursor:pointer; margin-top:2px;">
                <span style="flex:1; word-break:break-all; line-height:1.4;" title="${item.aweme_name || '未命名'}（ID: ${item.aweme_id}）">${item.aweme_name || '未命名'}</span>
            `;
